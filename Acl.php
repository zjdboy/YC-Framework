 ...
 * @version $Id: Acl.php 3 2011-12-29 15:01:09Z zjdboy $
 **/
class YC_Acl extends YC {
    ...
    public function __construct() {
        ...
        $this->aclcfg = array(
            ...
            'deMod' => 'default', //默认模块
            'sufObj' => 'Controller', //Controller后缀[对象的后缀]
            'sufLib' => 'Controller', //Controller文件后缀[对象的后缀]
            'sufFunc' => 'Action', //Action后缀[对象内方法的后缀]
    ...
    public function run($app) {
        ...
        if (is_file($fclass)) {
            ...
            throw new YC_Acl_Exception("Not Found Object: {$controller}", 404);
        }
        $this->cfg['debug'] && YC_Debug::factory()->dump();
    }
    ...
    public function _isModule(&$module, &$controller, &$path) {
        ...
        if (! isset($this->_module[$module])) {
            throw new YC_Acl_Exception("Not Found Modules: {$module}", 403);
        } elseif (isset($this->_module[$module]['controller']) && ! in_array($controller, $this->_module[$module]['controller'])) {
            throw new YC_Acl_Exception("Not Found Controller: {$controller}", 403);
        }
    ...
    private function _getRoute() {
        if (null === $this->_routeobj) {
            $this->_routeobj = new YC_Acl_Router();
        }
