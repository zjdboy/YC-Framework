 ...
 * @version $Id: Cache.php 3 2011-12-29 15:01:09Z zjdboy $
 **/
! defined('YC_CACHE_TYPE') && define('YC_CACHE_TYPE', 0);
//默认选用文件缓存
class YC_Cache {
    ...
    public static function factory($t) {
        if ($t == 1) {
            self::$in = YC_Cache_Memcache::Factory();
        } elseif ($t == 2) {
            self::$in = YC_Cache_Redis::Factory();
        } else {
            self::$in = YC_Cache_Fcache::Factory();
        }
    }
...
YC_Cache::factory(YC_CACHE_TYPE);
?>